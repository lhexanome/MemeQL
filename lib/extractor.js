...
async function extract(text) {
    try {
        ...
        results.forEach(res => {
            data[res.retrieved_url] = res.entities
                .filter(ent => ent.disambiguation && ent.disambiguation.dbpedia_resource)
                .map(ent => ent.disambiguation.dbpedia_resource)
                .concat((res.concepts || [])
                    .filter(c => c.dbpedia_resource)
                    .map(c => c.dbpedia_resource))
        });
